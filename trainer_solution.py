...
def get_loss_and_accuracy(logits, targets, eq_positions, mask, reduction='mean'):
    ...
    # Create a boolean mask for positions after the '=' token in each example.
    seq_range = torch.arange(S, device=device).unsqueeze(0).expand(B, S)
    rhs_mask = seq_range > eq_positions.unsqueeze(1)  # shape (B, S)
    
    # Combine with the provided padding mask (converted to bool).
    final_mask = mask.bool() & rhs_mask

    # Compute the per-token negative log-likelihood loss.
    # Flatten logits and targets to apply cross-entropy only on valid positions.
    logits_flat = logits.view(-1, V)
    targets_flat = targets.view(-1)
    loss_tokens = F.cross_entropy(logits_flat, targets_flat, reduction='none').view(B, S)
    
    # Sum losses over the RHS tokens while avoiding division by zero.
    valid_counts = final_mask.sum(dim=1).float()
    valid_counts = torch.where(valid_counts == 0, torch.ones_like(valid_counts), valid_counts)
    loss_per_sample = (loss_tokens * final_mask.float()).sum(dim=1) / valid_counts
    ...
    # Compute accuracy: a sample is correct only if all valid RHS tokens are predicted correctly.
    preds = logits.argmax(dim=-1)
    correct = (preds == targets) & final_mask
    total_correct = correct.sum(dim=1)
    total_valid = final_mask.sum(dim=1)
    # If there are no valid tokens, consider the sample as correct.
    sample_accuracy = torch.where(total_valid > 0, total_correct == total_valid, torch.ones_like(total_valid, dtype=torch.bool)).float()
