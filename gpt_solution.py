...
class MultiHeadedAttention(nn.Module):
    ...
    def get_attention_weights(self, queries, keys):
        ...
        # Compute scaled dot-product attention scores: (B, num_heads, S, S)
        d_k = self.head_size
        scores = torch.matmul(queries, keys.transpose(-2, -1)) / math.sqrt(d_k)
        
        # Create a causal mask (S, S) and apply it via broadcasting over batch and heads dimensions
        seq_length = scores.size(-1)
        causal_mask = torch.tril(torch.ones(seq_length, seq_length, dtype=torch.bool, device=scores.device))
        scores = scores.masked_fill(~causal_mask, float('-inf'))
        
        # Compute softmax over the last dimension to obtain the attention weights
    ...
    def apply_attention(self, queries, keys, values):
        ...
        """
        # Compute the attention weights using the existing function
        attn_weights = self.get_attention_weights(queries, keys)
        # Compute the weighted sum of the values
        context = torch.matmul(attn_weights, values)
        # Merge the heads to concatenate the outputs together
    ...
    def split_heads(self, tensor):
        ...
        batch_size, seq_length, embed_dim = tensor.size()
        tensor = tensor.view(batch_size, seq_length, self.num_heads, self.head_size)
        tensor = tensor.transpose(1, 2)  # shape: (batch_size, num_heads, sequence_length, head_size)
        return tensor
    ...
    def merge_heads(self, tensor):
        ...
        tensor = tensor.transpose(1, 2)  # change shape to (batch_size, sequence_length, num_heads, dim)
        return tensor.contiguous().view(batch_size, seq_length, num_heads * dim)
    ...
    def forward(self,  queries: Tensor, keys: Tensor, values: Tensor):
        ...
        # Project inputs to multi-head Q, K, and V representations
        ...
        # Use clone().detach() to detach attn_weights from the computation graph if needed
...
class GPTEmbedding(nn.Module):
    ...
    def create_sinusoidal_embeddings(self, n_positions: int, dimension: int) -> torch.FloatTensor:
        ...
        positions = torch.arange(n_positions, dtype=torch.float).unsqueeze(1)  # (n_positions, 1)
    ...
    def forward(self, tokens: Tensor) -> Tensor:
        ...
        position_encodings = self.position_encoding[:seq_length, :].unsqueeze(0).expand(batch_size, -1, -1)
        return token_embeddings + position_encodings
...
class GPT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tuple[Tensor, Tensor]]:
        ...
        out, (hidden_states, attentions) = self.decoder(embeddings)
