...
class LSTM(nn.Module):
    ...
    def count_parameters(self, bias=True):
        """
        Computes the total number of learnable parameters in the LSTM module,
        as a function of input dimension (dx), hidden dimension (dh),
        and number of layers (L). Here, dx = self.input_size,
        dh = self.hidden_size, and L = self.num_layers.
        
        For the first layer:
            Weights: 4 * ((dx + dh) * dh)
            Biases (if bias == True): 4 * dh
            => P₀ = 4 * ((dx + dh) * dh + (bias? dh : 0))
        
        For layers 1 to L-1:
            Weights: 4 * ((dh + dh) * dh) = 4 * (2 * dh^2)
            Biases (if bias == True): 4 * dh
            => Pᵢ = 4 * (2 * dh^2 + (bias? dh : 0))
        
        Total parameters:
            P = P₀ + (L - 1) * Pᵢ
        """
        first_layer = 4 * ((self.input_size + self.hidden_size) * self.hidden_size + (self.hidden_size if bias else 0))
        subsequent = 4 * ((2 * self.hidden_size * self.hidden_size) + (self.hidden_size if bias else 0))
        return first_layer + (self.num_layers - 1) * subsequent
    ...
    def forward(self, x, hx=None):
        ...
        output = x  # Use the input as the initial input for the first layer.
        final_hidden_states = []
        final_cell_states = []
        ...
        for layer_idx, cell in enumerate(self.layers):
            ...
            for t in range(seq_len):
                x_t = output[:, t, :]  # (batch_size, input_size) for first layer; (batch_size, hidden_size) for others
                h_t, c_t = cell(x_t, (h_t, c_t))
                layer_outputs.append(h_t.unsqueeze(1))  # Append along time dimension
...
class LSTMLM(nn.Module):
    ...
    def forward(self, x, hidden_states=None):
        ...
        hidden_states (`tuple` of size 2)
            ...
            - c (`torch.FloatTensor` of shape `(num_layers, batch_size, hidden_size)`)
        """
        # Obtain embeddings for the input token indices: (batch_size, seq_len, embedding_size)
        embedded = self.embedding(x)
        
        # Run the embedded sequence through the LSTM
        outputs, (h, c) = self.lstm(embedded, hidden_states)
        
        # Compute logits for the next token prediction: (batch_size, seq_len, vocabulary_size)
